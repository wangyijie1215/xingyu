#requires -Version 5
# EchoFlow art batch 2: remaining assets, lower cost (VRAM pressure on 8GB).
...
function New-Workflow {
  param($Pos, $W, $H, $Seed, $Prefix, $Lora, $LoraStrength, $Steps)
  return [ordered]@{
    ...
    '9'  = @{ class_type = 'SaveImage'; inputs = @{ filename_prefix = "ef2/$Prefix"; images = @('8', 0) } }
  }
}
...
$assets = @(
  @{ n='alice-sad'; w=768; h=1120; s=900001; lora=$DETAIL; ls=0.55; st=26;
     p="$ALICE, sad expression, downcast eyes, quiet and lonely mood, soft melancholy lighting" },
  @{ n='alice-tired'; w=768; h=1120; s=900001; lora=$DETAIL; ls=0.55; st=26;
     p="$ALICE, tired expression, half closed sleepy eyes, exhausted but calm, late night mood" },
  @{ n='rin-portrait'; w=768; h=1120; s=910001; lora=$DETAIL; ls=0.5; st=26;
  ...
  @{ n='hakuyo-portrait'; w=768; h=1120; s=910002; lora=$DETAIL; ls=0.5; st=26;
  ...
  @{ n='rocco-portrait'; w=768; h=1120; s=910003; lora=$DETAIL; ls=0.5; st=26;
  ...
  @{ n='campus-autumn'; w=1152; h=768; s=920001; lora=$ANIME; ls=0.5; st=26;
  ...
  @{ n='chat-bg-alice'; w=1152; h=768; s=930001; lora=$DETAIL; ls=0.5; st=26;
...
Write-Output ("Batch2: queueing {0} jobs..." -f $assets.Count)
$jobs = New-Object System.Collections.ArrayList
foreach ($a in $assets) {
  $wf = New-Workflow -Pos $a.p -W $a.w -H $a.h -Seed $a.s -Prefix $a.n -Lora $a.lora -LoraStrength $a.ls -Steps $a.st
  $body = @{ prompt = $wf; client_id = 'echoflow-batch2' } | ConvertTo-Json -Depth 24 -Compress
  try {
...
Write-Output ("Batch2 finished: {0}/{1}" -f $done, $jobs.Count)
